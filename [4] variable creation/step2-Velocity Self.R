...
# 1.Creat two files (same as in file"expert variable Risk, Last seen code.R")
...
#2.Adjusting columns order (same as in file"expert variable Risk, Last seen code.R")
...
#counting how many records seen over last n days
...
# separately process n=30, because at first we don't know wether we want to use n=30
...
### iii.organize to a pretty dataset for "Velovity" variables
...
# 4.join to generate one dataset (same as in file"expert variable Risk, Last seen code.R")
#--join final table of "days since variable creation" and table of "riak variable creation"
...
variables <- read.csv('FinalNewVariables.csv') #There is NA in it. replace NA with 365 before feature engineering
