...
df$date <- ymd(df$date)
df1 <- df[df$date <= '2016-10-31',] # get traning+test set
rm(col)
...
## (3)create 'Risk table' variables

#temp <- df1 %>%
  #group_by(ssn) %>%
  #summarise(risk = mean(fraud_label))

### i.create a function for the entity and used it to each entity
...
for(entity in entities){ #create a variable(table) for each entity
  assign(paste0('rt_', entity), risk_table(df1, entity))
}

###ii. create a function for the combinations and used it to each combination
risk_table <- function(we, entity){
  temp <- we %>%
...
### iii. join numerical variables created in the i. and ii. steps on dataset in hand
entities <- c('ssn','fulladdress', 'nameDOB', 'homephone')
...
# 4.join to generate one dataset
...
setDT(risk)
#setDT(velocity)
variables <- last_time[risk, on = 'record']  #join two datasets from last 2 main steps( 3.(1)and(2) )
